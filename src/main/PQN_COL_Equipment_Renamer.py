...
class RenamerApp(ctk.CTk):
   ...
   def build_ui(self):
      ...
      main_frame.pack(fill="both", expand=True, padx=10, pady=10)  # Reducimos el padding del marco principal.
      ...
      header_frame.pack(fill="x", pady=(0, 10))  # Reducimos el espacio inferior del encabezado.
      ...
      info_title.pack(pady=(10, 5), anchor="w", padx=15)  # Reducimos el padding alrededor del título.
      
      # Reducción del espaciado entre las etiquetas de la información
      self.info_serial = ctk.CTkLabel(info_frame, text="Serial BIOS: Obteniendo información...", font=FONT_INFO, text_color=COLOR_TEXT_WHITE, anchor="w")
      self.info_serial.pack(pady=3, padx=15, anchor="w")
      
      self.info_manufacturer = ctk.CTkLabel(info_frame, text="Fabricante: Obteniendo información...", font=FONT_INFO, text_color=COLOR_TEXT_WHITE, anchor="w")
      self.info_manufacturer.pack(pady=3, padx=15, anchor="w")
      
      self.info_model = ctk.CTkLabel(info_frame, text="Modelo: Obteniendo información...", font=FONT_INFO, text_color=COLOR_TEXT_WHITE, anchor="w")
      self.info_model.pack(pady=3, padx=15, anchor="w")
      
      self.info_current_name = ctk.CTkLabel(info_frame, text="Nombre actual: Obteniendo información...", font=FONT_INFO, text_color=COLOR_TEXT_WHITE, anchor="w")
      self.info_current_name.pack(pady=3, padx=15, anchor="w")
      
      self.info_domain = ctk.CTkLabel(info_frame, text="Dominio/Grupo: Obteniendo información...", font=FONT_INFO, text_color=COLOR_TEXT_WHITE, anchor="w")
      self.info_domain.pack(pady=(3, 10), padx=15, anchor="w")  # Reducimos el espaciado aquí también.
      ...
      config_frame.pack(fill="x", pady=(0, 10))  # Reducimos el espaciado entre marcos.
      ...
      site_buttons_frame.pack(pady=(0, 10), padx=15, anchor="w")  # Reducimos el espaciado entre los botones.
      ...
      for site in SITE_OPTIONS:
         ...
         ).pack(side="left", padx=(0, 15))  # Reducimos el padding entre los botones de selección.
      ...
      preview_frame.pack(fill="x", pady=(0, 10))  # Reducimos el espaciado entre marcos.
      ...
      self.preview_name.pack(pady=(0, 5), padx=15, anchor="w")  # Reducimos el espaciado entre el texto.
      ...
      self.preview_validation.pack(pady=(0, 10), padx=15, anchor="w")  # Reducimos el espaciado.
      ...
      log_label.pack(pady=(5, 5), anchor="w")  # Reducimos el espaciado entre el título y el log.
      ...
      self.text_log.pack(pady=(0, 10))  # Reducimos el espaciado entre la caja de texto y la parte inferior.
      ...
      self.status_label.pack(pady=(0, 10))  # Reducimos el espaciado entre la barra de estado y la parte inferior.
      
      # === BOTONES ===
      button_frame = ctk.CTkFrame(main_frame, fg_color="transparent")
      button_frame.pack(fill="x", pady=(10, 5))  # Reducimos el espaciado entre los botones.
      ...
      self.btn_execute.pack(side="left", expand=True, fill="x", padx=(0, 5))  # Reducimos el padding lateral.
      ...
      self.btn_export.pack(side="left", expand=True, fill="x", padx=(5, 0))  # Reducimos el padding lateral y el espaciado entre botones.
   ...
   def check_prerequisites(self):
      ...
      try:
         success, output, _ = run_powershell("(Get-WmiObject Win32_OperatingSystem).Caption")
         if success:
   ...
   def update_preview(self):
      ...
      if not new_name:
         self.preview_name.configure(text="ERROR: Nombre inválido", text_color=COLOR_ERROR)
         self.preview_validation.configure(
               text="✗ No se pudo generar un nombre válido",
   ...
   def export_log(self):
      ...
      try:
         ...
         export_path = desktop / f"Renamer_Log_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
   ...
   def on_execute(self):
      ...
      confirmation_msg += f"• El equipo se reiniciará automáticamente en 15 segundos\n"
   ...
   def execute_rename(self, new_name, site):
      ...
      try:
         ...
         if need_rename:
               self.log(f"      ✓ Cambio de nombre requerido: {current_name} → {new_name}", "SUCCESS")
         else:
         ...
         if site == "PQN" and need_domain:
               ...
               self.log(f"      → Uniendo al dominio '{DOMAIN_NAME}' con nombre '{new_name}'...", "INFO")
               
               success = join_domain(DOMAIN_NAME, DOMAIN_USER, DOMAIN_PASSWORD, new_name)
